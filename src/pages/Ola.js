...
import olabanner from "../pages/images/olabanner.jpeg";
...
export const Ola = ({ handleclick }) => {
  ...
  const [cycleType, setCycleList] = useState("");

  async function getCycleList() {
    let resultCycle = await axios.get(
      `https://app.fuelfree.in/product/brand?Brand=ola`,
      {
        headers: {
    ...
    let alldata = await cycleData.searchedProduct;
    setCycleList(alldata);
  }
  ...
  const getfilter = async () => {
    ...
    let res = await axios.get(
      `https://app.fuelfree.in/product/brand?Brand=OLA&${DrivingRangeProduct}${maxPrice}`,
      {
        headers: {
          Accept: "application/json",
        },
  ...
  return (
    <div>
      <Header />
      <img src={olabanner} alt="MercedesBenz motors"></img>
